  ...
  const executeFullArbitrage = useCallback(
    async (opp: ArbitrageOpportunity) => {
      ...
      try {
        ...
        addLog(
          `  Step 1: Flash borrow ${opp.borrowAmount} ${borrowPoolInfo.baseCoin}...`,
        );
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Atomic arbitrage using DeepBook V3 flash loans
            </p>
          </div>
        ...
        <div className="mb-5 p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
          ...
          <ol className="text-xs text-muted-foreground space-y-0.5 list-decimal list-inside">
            <li>
              Borrow assets from DeepBook pool via flash loan (no collateral)
            </li>
            ...
            <li className="text-chart-3">
              All steps MUST complete in one transaction
            </li>
          </ol>
        </div>

        {/* Flash Loan Config */}
        <div className="bg-muted/50 rounded-lg p-4 sm:p-5 border border-border mb-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
            ...
            <div>
              ...
              <select
                value={borrowAsset}
                onChange={(e) =>
                  setBorrowAsset(e.target.value as "base" | "quote")
                }
                className="w-full bg-background border border-border rounded-lg px-3 py-2 text-sm outline-none focus:border-primary"
              >
                <option value="base">Base ({borrowPoolInfo?.baseCoin})</option>
                <option value="quote">
                  Quote ({borrowPoolInfo?.quoteCoin})
                </option>
              </select>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              ...
              className="flex-1 py-2.5 bg-primary hover:bg-primary rounded-lg font-semibold text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isPending ? "Executing..." : "Execute Flash Loan"}
            </Button>
            <Button
              onClick={scanOpportunities}
              className="px-4 py-2.5 bg-muted hover:bg-muted rounded-lg font-medium text-sm transition-colors"
            >
              Scan
            </Button>
          </div>
        ...
        {opportunities.length > 0 && (
          <div className="bg-muted/50 rounded-lg p-4 sm:p-5 border border-border mb-5">
              ...
              {opportunities.map((opp) => (
                ...
                >
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span
                      className={`text-xs ${opp.profitPercent > 0.5 ? "text-chart-2" : "text-chart-3"}`}
                    >
                  ...
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    <span>
                      Borrow: {opp.borrowPool} | Swap: {opp.swapPool}
                    </span>
                    <Button
                      onClick={() => executeFullArbitrage(opp)}
                      disabled={isPending}
                      className="px-2.5 py-1 bg-primary/20 text-primary rounded hover:bg-primary/30 disabled:opacity-50"
                    >
                      Execute
                    </Button>
                  </div>
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              Note: Simulated profits. Real arbitrage requires pools with
              liquidity.
            </p>
          </div>
        )}
        ...
        {/* Activity Log */}
        <div className="bg-muted/50 rounded-lg p-4 border border-border mb-5">
          ...
          <div className="bg-background/50 rounded-lg p-2.5 h-40 overflow-y-auto font-mono text-xs">
            {logs.length === 0 ? (
              <p className="text-muted-foreground">
                No activity yet. Execute a flash loan to see logs...
              </p>
            ) : (
              logs.map((log, i) => (
        ...
        {/* Last Transaction */}
        {lastTx && (
          <div className="p-3 bg-chart-2/10 border border-green-500/20 rounded-lg mb-5">
        ...
        <div className="text-center text-xs text-muted-foreground space-y-1">
          <p>
            Flash loans allow you to borrow without collateral within a single
            transaction.
          </p>
          <p className="text-chart-3">
            Note: Testnet pools often lack liquidity for arbitrage.
          </p>
        </div>
