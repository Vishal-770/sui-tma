...
const STATUS_CONFIG: Record<number, { label: string; color: string; bg: string }> = {
  0: { label: 'Active', color: 'rgb(34, 197, 94)', bg: 'rgba(34, 197, 94, 0.1)' },
  1: { label: 'Executing', color: 'rgb(14, 165, 233)', bg: 'rgba(14, 165, 233, 0.1)' },
  2: { label: 'Executed', color: 'rgb(34, 197, 94)', bg: 'rgba(34, 197, 94, 0.1)' },
  3: { label: 'Cancelled', color: 'rgb(156, 163, 175)', bg: 'rgba(156, 163, 175, 0.1)' },
  4: { label: 'Expired', color: 'rgb(234, 179, 8)', bg: 'rgba(234, 179, 8, 0.1)' },
  5: { label: 'Failed', color: 'rgb(239, 68, 68)', bg: 'rgba(239, 68, 68, 0.1)' },
};
      ...
      <div className="w-full max-w-4xl mx-auto px-4 py-6 flex flex-col gap-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Link href="/intents/create" className="p-2 bg-sky-500 hover:bg-sky-400 rounded-lg transition-colors">
        ...
        {!isZkLogin && (
          <div className="p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
            <p className="text-yellow-400 text-sm">
              You're connected with a wallet. To create or cancel intents, you need to sign in with zkLogin.
            </p>
            <Link href="/login" className="text-yellow-300 text-sm underline mt-1 inline-block">
              Switch to zkLogin
            </Link>
          </div>
        )}
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="bg-gray-900/50 rounded-xl p-4 border border-gray-800 text-center">
            <span className="text-2xl font-bold text-white">{intents.length}</span>
          ...
          <div className="bg-gray-900/50 rounded-xl p-4 border border-gray-800 text-center">
            <span className="text-2xl font-bold text-gray-400">{completedCount}</span>
          ...
          {filteredIntents.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center py-16">
              ...
              {filter === 'all' && isZkLogin && (
                <Link href="/intents/create" className="px-6 py-2.5 bg-sky-500 hover:bg-sky-400 rounded-lg font-medium text-sm transition-colors">
                  Create Intent
                </Link>
              )}
            </div>
                  ...
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-semibold text-white">{intent.pair.replace('_', '/')}</span>
                        <span 
                    ...
                    {intent.side && (
                      <span className={`font-semibold ${intent.side === 'buy' ? 'text-green-400' : 'text-red-400'}`}>
                    ...
                    <div className="flex gap-4 text-sm">
                      {intent.quantity && (
                        <span>
                          ...
                          <span className="text-gray-300 font-medium">{intent.quantity}</span>
                        </span>
                      )}
                      <span>
